...
static void convolutional_sse_decode_inner(correct_convolutional_sse *sse_conv,
                                           unsigned int sets,
                                           const uint8_t *soft) {
    ...
    for (unsigned int i = conv->order - 1; i < (sets - conv->order + 1); i++) {
        ...
        if (soft) {
            if (conv->soft_measurement == CORRECT_SOFT_LINEAR) {
                for (unsigned int j = 0; j < 1 << (conv->rate); j++) {
                    distances[j] = metric_soft_distance_linear(
                        j, soft + i * conv->rate, conv->rate);
                }
            } else {
                for (unsigned int j = 0; j < 1 << (conv->rate); j++) {
                    distances[j] = metric_soft_distance_quadratic(
                        j, soft + i * conv->rate, conv->rate);
                }
            }
        } else {
        ...
        for (shift_register_t low = 0, high = highbit, base = 0, oct = 0;
             high < num_iter; low += 32, high += 32, base += 16, oct += 4) {
            ...
            __v4si past_shuffle_mask =
                (__v4si){0x01000100, 0x03020302, 0x05040504, 0x07060706};
            __v4si hist_mask =
                (__v4si){0x07050301, 0x0e0c0a09, 0x80808080, 0x80808080};
            ...
            for (shift_register_t offset = 0, base_offset = 0;
                ...
                // a past error
                low_past_error =
                    _mm_shuffle_epi8(low_past_error, past_shuffle_mask);
                low_past_error0 =
                    _mm_shuffle_epi8(low_past_error0, past_shuffle_mask);
                low_past_error1 =
                    _mm_shuffle_epi8(low_past_error1, past_shuffle_mask);
                low_past_error2 =
                    _mm_shuffle_epi8(low_past_error2, past_shuffle_mask);

                // repeat past error lookup for register states with high order
                // bit set
                __v4si high_past_error = _mm_loadl_epi64((const __m128i *)(
                    read_errors + highbase + base + base_offset));
                __v4si high_past_error0 = _mm_loadl_epi64((const __m128i *)(
                    read_errors + highbase + base + base_offset + 4));
                __v4si high_past_error1 = _mm_loadl_epi64((const __m128i *)(
                    read_errors + highbase + base + base_offset + 8));
                __v4si high_past_error2 = _mm_loadl_epi64((const __m128i *)(
                    read_errors + highbase + base + base_offset + 12));

                high_past_error =
                    _mm_shuffle_epi8(high_past_error, past_shuffle_mask);
                high_past_error0 =
                    _mm_shuffle_epi8(high_past_error0, past_shuffle_mask);
                high_past_error1 =
                    _mm_shuffle_epi8(high_past_error1, past_shuffle_mask);
                high_past_error2 =
                    _mm_shuffle_epi8(high_past_error2, past_shuffle_mask);

                // __v4si this_shuffle_mask = (__v4si){0x80800100, 0x80800302,
                ...
                distance_oct_key_t low_key0 =
                    oct_lookup.keys[oct + (base_offset / 4) + 1];
                distance_oct_key_t low_key1 =
                    oct_lookup.keys[oct + (base_offset / 4) + 2];
                distance_oct_key_t low_key2 =
                    oct_lookup.keys[oct + (base_offset / 4) + 3];

                // load the distances for the register states with high order
                // bit cleared
                __v4si low_this_error =
                    _mm_load_si128((const __m128i *)(oct_lookup.distances + low_key));
                __v4si low_this_error0 =
                    _mm_load_si128((const __m128i *)(oct_lookup.distances + low_key0));
                __v4si low_this_error1 =
                    _mm_load_si128((const __m128i *)(oct_lookup.distances + low_key1));
                __v4si low_this_error2 =
                    _mm_load_si128((const __m128i *)(oct_lookup.distances + low_key2));

                // add the distance for this time slice to the past distances
                __v4si low_error =
                    _mm_add_epi16(low_past_error, low_this_error);
                __v4si low_error0 =
                    _mm_add_epi16(low_past_error0, low_this_error0);
                __v4si low_error1 =
                    _mm_add_epi16(low_past_error1, low_this_error1);
                __v4si low_error2 =
                    _mm_add_epi16(low_past_error2, low_this_error2);
                ...
                __v4si high_this_error =
                    _mm_load_si128((const __m128i *)(oct_lookup.distances + high_key));
                __v4si high_this_error0 =
                    _mm_load_si128((const __m128i *)(oct_lookup.distances + high_key0));
                __v4si high_this_error1 =
                    _mm_load_si128((const __m128i *)(oct_lookup.distances + high_key1));
                __v4si high_this_error2 =
                    _mm_load_si128((const __m128i *)(oct_lookup.distances + high_key2));

                __v4si high_error =
                    _mm_add_epi16(high_past_error, high_this_error);
                __v4si high_error0 =
                    _mm_add_epi16(high_past_error0, high_this_error0);
                __v4si high_error1 =
                    _mm_add_epi16(high_past_error1, high_this_error1);
                __v4si high_error2 =
                    _mm_add_epi16(high_past_error2, high_this_error2);

                // distances for this time slice calculated

                // find the least error between registers who differ only in
                // their high order bit
                __v4si min_error = _mm_min_epu16(low_error, high_error);
                __v4si min_error0 = _mm_min_epu16(low_error0, high_error0);
                __v4si min_error1 = _mm_min_epu16(low_error1, high_error1);
                __v4si min_error2 = _mm_min_epu16(low_error2, high_error2);
                ...
                //      error
                __v4si hist = _mm_cmpgt_epi16(low_error, min_error);
                // pack the bits down from 16-bit wide to 8-bit wide to
                // accomodate history table
                hist = _mm_shuffle_epi8(hist, hist_mask);

                __v4si hist0 = _mm_cmpgt_epi16(low_error0, min_error0);
                hist0 = _mm_shuffle_epi8(hist0, hist_mask);

                __v4si hist1 = _mm_cmpgt_epi16(low_error1, min_error1);
                hist1 = _mm_shuffle_epi8(hist1, hist_mask);

                __v4si hist2 = _mm_cmpgt_epi16(low_error2, min_error2);
        ...
        if (hist_buf_len == hist_buf_cap - 1 || hist_buf_rn_cnt == hist_buf_rn_int - 1) {
            ...
            history_buffer_process(conv->history_buffer, write_errors,
                                   conv->bit_writer);
...
static void _convolutional_sse_decode_init(correct_convolutional_sse *conv,
    ...
    conv->oct_lookup = oct_lookup_create(
        conv->base_conv.rate, conv->base_conv.order, conv->base_conv.table);
}
...
static size_t _convolutional_sse_decode(correct_convolutional_sse *sse_conv,
    ...
    if (!conv->has_init_decode) {
        ...
        _convolutional_sse_decode_init(sse_conv, 5 * conv->order, 100 * conv->order, renormalize_interval);
    }
...
size_t correct_convolutional_sse_decode(correct_convolutional_sse *conv,
    ...
    size_t num_encoded_bytes = (num_encoded_bits % 8)
                                   ? (num_encoded_bits / 8 + 1)
                                   : (num_encoded_bits / 8);
    bit_reader_reconfigure(conv->base_conv.bit_reader, encoded, num_encoded_bytes);

    return _convolutional_sse_decode(conv, num_encoded_bits, num_encoded_bytes,
                                     msg, NULL);
}

size_t correct_convolutional_sse_decode_soft(correct_convolutional_sse *conv,
                                             const soft_t *encoded,
                                             size_t num_encoded_bits,
                                             uint8_t *msg) {
    if (num_encoded_bits % conv->base_conv.rate) {
        // XXX turn this into an error code
